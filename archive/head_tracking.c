...
int main() {
    ...
    while (1) {
        ...
        if (nbytes < 0) {
            ...
            char errbuf[256];
            iio_strerror(-nbytes, errbuf, sizeof(errbuf));
            fprintf(stderr, "Buffer refill failed: %s (%zd)\n", errbuf, nbytes);
            //break; // Removed temporarily CT 7/8/26
        }
        float gravity[3] = {0};
